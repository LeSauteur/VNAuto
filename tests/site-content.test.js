...
test('карточки услуг не растягивают пустую зону перед CTA', () => {
  assert.match(css, /\.service-card\s*\{[^}]*min-height:\s*0/s);
  assert.match(css, /\.service-card a\s*\{[^}]*margin-top:\s*18px/s);
  assert.doesNotMatch(css, /\.service-card a\s*\{[^}]*margin-top:\s*auto/s);
});
